...
func (s *PublicEthereumAPI) FeeHistory(ctx context.Context, blockCount rpc.DecimalOrHex, lastBlock rpc.BlockNumber, rewardPercentiles []float64) (*feeHistoryResult, error) {
	...
	res.Note = `On Arthera, the eth_feeHistory method operates slightly differently due to the network's unique consensus mechanism. ` +
...
func (s *PublicTxPoolAPI) Content() map[string]map[string]map[string]*RPCTransaction {
	...
	for account, txs := range pending {
		...
		for _, tx := range txs {
			dump[fmt.Sprintf("%d", tx.Nonce())] = newRPCPendingTransaction(tx, curHeader.BaseFee, s.b.ChainConfig())
		}
...
func (s *PublicBlockChainAPI) GetRules(ctx context.Context, epoch rpc.BlockNumber) (*params.ProtocolRules, error) {
...
func DoCall(ctx context.Context, b Backend, args TransactionArgs, blockNrOrHash rpc.BlockNumberOrHash, overrides *StateOverride, timeout time.Duration, globalGasCap uint64) (*evmcore.ExecutionResult, error) {
	...
	result, err := evmcore.ApplyMessageWithoutGasPrice(evm, msg, gp)
...
// transaction hashes.
func RPCMarshalBlock(block *evmcore.EvmBlock, ext extBlockApi, inclTx bool, fullTx bool, config *ethparams.ChainConfig) (map[string]interface{}, error) {
...
func (s *PublicBlockChainAPI) rpcMarshalBlock(b *evmcore.EvmBlock, ext extBlockApi, inclTx bool, fullTx bool) (map[string]interface{}, error) {
	fields, err := RPCMarshalBlock(b, ext, inclTx, fullTx, s.b.ChainConfig())
	if err != nil {
		return nil, err
	}
...
func newRPCTransaction(tx *types.Transaction, blockHash common.Hash, blockNumber uint64, index uint64, baseFee *big.Int, config *ethparams.ChainConfig) *RPCTransaction {
...
func newRPCPendingTransaction(tx *types.Transaction, baseFee *big.Int, config *ethparams.ChainConfig) *RPCTransaction {
	return newRPCTransaction(tx, common.Hash{}, 0, 0, baseFee, config)
}

// newRPCTransactionFromBlockIndex returns a transaction that will serialize to the RPC representation.
func newRPCTransactionFromBlockIndex(b *evmcore.EvmBlock, index uint64, config *ethparams.ChainConfig) *RPCTransaction {
	txs := b.Transactions
...
func newRPCTransactionFromBlockHash(b *evmcore.EvmBlock, hash common.Hash, config *ethparams.ChainConfig) *RPCTransaction {
...
func (s *PublicTransactionPoolAPI) GetTransactionByHash(ctx context.Context, hash common.Hash) (*RPCTransaction, error) {
	...
	if tx != nil {
		...
		return newRPCTransaction(tx, header.Hash, blockNumber, index, header.BaseFee, s.b.ChainConfig()), nil
	}
	// No finalized transaction, try to retrieve it from the pool
	if tx := s.b.GetPoolTransaction(hash); tx != nil {
		return newRPCPendingTransaction(tx, s.b.MinGasPrice(), s.b.ChainConfig()), nil
	}
...
func SubmitTransaction(ctx context.Context, b Backend, tx *types.Transaction) (common.Hash, error) {
	...
	if tx.To() == nil {
		...
		log.Trace("Submitted contract creation", "hash", tx.Hash().Hex(), "from", from, "nonce", tx.Nonce(), "contract", addr.Hex(), "value", tx.Value(), "gas", tx.Gas())
	} else {
		log.Trace("Submitted transaction", "hash", tx.Hash().Hex(), "from", from, "nonce", tx.Nonce(), "recipient", tx.To(), "value", tx.Value(), "gas", tx.Gas())
	}
	return tx.Hash(), nil
}
...
func (s *PublicTransactionPoolAPI) PendingTransactions() ([]*RPCTransaction, error) {
	...
	for _, tx := range pending {
		...
		if _, exists := accounts[from]; exists {
			transactions = append(transactions, newRPCPendingTransaction(tx, s.b.MinGasPrice(), s.b.ChainConfig()))
		}
	}
...
func (api *PublicDebugAPI) traceTx(ctx context.Context, message evmcore.Message, txctx *tracers.Context, vmctx vm.BlockContext, statedb *state.StateDB, config *TraceConfig) (interface{}, error) {
	...
	case config.Tracer != nil:
		...
		if t, err := tracers.DefaultDirectory.New(*config.Tracer, txctx, nil); err != nil {
			return nil, err
		} else {
...
func (api *PublicDebugAPI) traceBlock(ctx context.Context, block *evmcore.EvmBlock, config *TraceConfig) ([]*txTraceResult, error) {
	...
	for i, tx := range txs {
		...
		vmenv := vm.NewEVM(blockCtx, evmcore.NewEVMTxContext(msg), statedb, api.b.ChainConfig(), params.DefaultVMConfig)
...
func (api *PublicDebugAPI) stateAtTransaction(ctx context.Context, block *evmcore.EvmBlock, txIndex int) (evmcore.Message, vm.BlockContext, *state.StateDB, error) {
	...
	for idx, tx := range block.Transactions {
		...
		vmenv := vm.NewEVM(context, txContext, statedb, api.b.ChainConfig(), params.DefaultVMConfig)
