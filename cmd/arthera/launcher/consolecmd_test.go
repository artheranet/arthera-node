...
const (
	ipcAPIs  = "abft:1.0 admin:1.0 dag:1.0 debug:1.0 art:1.0 net:1.0 personal:1.0 rpc:1.0 trace:1.0 txpool:1.0 web3:1.0"
	httpAPIs = "abft:1.0 dag:1.0 art:1.0 rpc:1.0 web3:1.0"
)
...
func TestIPCAttachWelcome(t *testing.T) {
	...
	if runtime.GOOS == "windows" {
		ipc = `\\.\pipe\arthera.ipc`
	} else {
		ws := tmpdir(t)
		defer os.RemoveAll(ws)
		ipc = filepath.Join(ws, "arthera.ipc")
	}
	cli := exec(t,
		"--fakenet", "0/1", "--port", "0", "--maxpeers", "0", "--nodiscover", "--nat", "none",
		"--ipcpath", ipc)
