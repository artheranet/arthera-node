...
import (
	...
	"github.com/artheranet/arthera-node/cmd/cmdtest"
)

func tmpdir(t *testing.T) string {
	dir, err := ioutil.TempDir("", "arthera-test")
	if err != nil {
		t.Fatal(err)
	}
	return dir
...
func init() {
	// Run the app if we've been exec'd as "arthera-test" in exec().
	reexec.Register("arthera-test", func() {
...
func exec(t *testing.T, args ...string) *testcli {
	...
	// Boot "arthera". This actually runs the test binary but the TestMain
	// function will prevent any tests from running.
	tt.Run("arthera-test", args...)
