...
`,
			...
			{
				Name:   "addr",
				Usage:  "Print the address of the validator",
				Action: utils.MigrateFlags(validatorPrintAddress),
				Flags: []cli.Flag{
					utils.KeyStoreDirFlag,
				},
...
func validatorPrintAddress(ctx *cli.Context) error {
	cfg := makeAllConfigs(ctx)
	utils.SetNodeConfig(ctx, &cfg.Node)

	pubkeyStr := ctx.Args().Get(0)
	pubkey, err := validatorpk.FromString(pubkeyStr)
	if err != nil {
		utils.Fatalf("Failed to decode the validator pubkey: %v", err)
	}

	password := getPassPhrase("Type your password to unlock your validator key.", false, 0, utils.MakePasswordList(ctx))

	vks := valkeystore.NewDefaultFileRawKeystore(path.Join(getValKeystoreDir(cfg.Node), "validator"))
	privkey, err := vks.Get(pubkey, password)
	if err != nil {
		utils.Fatalf("Failed to decode the validator key: %v", err)
	}

	privKey := privkey.Decoded.(*ecdsa.PrivateKey)
