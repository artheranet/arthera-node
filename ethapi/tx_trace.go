...
func (s *PublicTxTraceAPI) traceTx(
	...
	status uint64, chainConfig *ethparams.ChainConfig) (*[]txtrace.ActionTrace, error) {

	// Providing default config
	// In case of trace transaction node, this config is changed
	cfg := params.DefaultVMConfig
	cfg.Debug = true
	...
	if !allTxOK {
		...
		for i, tx := range block.Transactions {
			if txHash == nil || *txHash == tx.Hash() {
				...
				from := msg.From()
				if tx.To() != nil && *tx.To() == contracts.StakingSmartContractAddress {
					errTrace := txtrace.GetErrorTrace(block.Hash, *block.Number, &from, tx.To(), tx.Hash(), index, errors.New("Staking tx"))
					at := make([]txtrace.ActionTrace, 0)
					at = append(at, *errTrace)
