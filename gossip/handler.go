...
// newHandler returns a new Arthera Beacon sub protocol manager. The Arthera sub protocol manages peers capable
// with the Arthera Beacon network.
func newHandler(
	c handlerConfig,
) (
	*handler,
...
func (h *handler) Stop() {
	log.Info("Stopping Arthera Beacon protocol")
	...
	log.Info("Arthera Beacon protocol stopped")
}
...
func (h *handler) handle(p *peer) error {
	...
	if p.RunningCap(ProtocolName, []uint{ART10}) {
