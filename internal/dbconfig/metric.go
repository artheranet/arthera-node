...
func (db *DBProducerWithMetrics) OpenDB(name string) (kvdb.Store, error) {
	...
	// the last db size metric will overwrite all the previous one
	dm.diskSizeGauge = metrics.GetOrRegisterGauge("arthera/chaindata/"+strings.ReplaceAll(name, "-", "_")+"/disk/size", nil)
	if strings.HasPrefix(name, "gossip-") || strings.HasPrefix(name, "lachesis-") || strings.HasPrefix(name, "epoch-") {
		name = "epochs"
	}
	logger := log.New("database", name)
	dm.log = logger
	dm.diskReadMeter = metrics.GetOrRegisterMeter("arthera/chaindata/"+strings.ReplaceAll(name, "-", "_")+"/disk/read", nil)
	dm.diskWriteMeter = metrics.GetOrRegisterMeter("arthera/chaindata/"+strings.ReplaceAll(name, "-", "_")+"/disk/write", nil)
