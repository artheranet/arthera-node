...
func (db *DBProducerWithMetrics) OpenDB(name string) (kvdb.Store, error) {
	...
	dm.log = logger
	dm.diskReadMeter = metrics.GetOrRegisterMeter("arthera/chaindata/"+name+"/disk/read", nil)
	dm.diskWriteMeter = metrics.GetOrRegisterMeter("arthera/chaindata/"+name+"/disk/write", nil)
