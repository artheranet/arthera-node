...
func NewTxPool(config TxPoolConfig, chainconfig *params.ChainConfig, chain StateReader) *TxPool {
	...
	for _, addr := range config.Locals {
		log.Trace("Setting new local account", "address", addr)
...
func (pool *TxPool) validateTx(tx *types.Transaction, local bool) error {
	...
	// Ensure Arthera-specific hard bounds
...
func (pool *TxPool) addTxs(txs []*types.Transaction, local, sync bool) []error {
	...
	for i, tx := range txs {
		...
		// Accvsumulate all unknown transactions for deeper processing
		news = append(news, tx)
	}
	if len(news) == 0 {
		return errs
...
func (pool *TxPool) reset(oldHead, newHead *EvmHeader) {
	// update chain config (Arthera-specific)
...
func (pool *TxPool) promoteExecutables(accounts []common.Address) []*types.Transaction {
	...
	for _, addr := range accounts {
		...
		drops, _ := list.Filter(pool.currentState.GetBalance(addr), pool.currentMaxGas, addr, pool.currentState, pool.currentVMRunner)
...
func (pool *TxPool) demoteUnexecutables() {
	...
	for addr, list := range pool.pending {
		...
		drops, invalids := list.Filter(pool.currentState.GetBalance(addr), pool.currentMaxGas, addr, pool.currentState, pool.currentVMRunner)
		for _, tx := range drops {
			hash := tx.Hash()
