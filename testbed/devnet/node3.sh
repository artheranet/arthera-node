...
"$SCRIPT_DIR/../../build/arthera-node" --devnet --genesis devnet.g --genesis.allowUnknown --port 6536 --netrestrict 127.0.0.1/8 \
  --identity "node3" --nodekey "$NODE_DIR/node.key" --nousb --nat=none --cache 8000 \
  ...
  --http.addr 0.0.0.0 --http.port 18547 --http --http.corsdomain "*" --http.vhosts="*" \
  --http.api=eth,web3,net,txpool,art,abft \
  --ws --ws.addr 0.0.0.0 --ws.port 18557 --ws.origins "*" --ws.api=eth,web3,net,txpool,art,abft
