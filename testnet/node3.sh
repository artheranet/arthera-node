...
"$SCRIPT_DIR/../build/arthera-node" --testnet --port 6536 --netrestrict 127.0.0.1/8 \
  --identity "node3" --nodekey "$NODE_DIR/node.key" \
  ...
  --http.api=eth,web3,net,txpool,art,abft --http.vhosts="*" \
  --http.addr 0.0.0.0 --http --http.corsdomain "*"
